...
class net(nn.Module):
    ...
    def reset_r2d2_head(self, xs, lam=50):
        self.encoder.eval()
        with torch.no_grad():
        ...
        state_dict = dict(weight=weight, bias=torch.zeros(self.n_way).cuda())
        self.head.load_state_dict(state_dict)

    def reset_metaoptnet_head(self, xs, C_reg=0.1, maxIter=15):
        self.encoder.eval()
        with torch.no_grad():
            support, _ = self.encoder(xs)
        num_support, d = support.size()
        ...
        # the learning rate is very small, no need to increase the weight
        state_dict = dict(weight=weight, bias=torch.zeros(self.n_way).cuda())
        self.head.load_state_dict(state_dict)
...
def finetune(novel_loader, n_way=5, n_support=5, n_query=15):
    ...
    for ti, (x, _) in enumerate(novel_loader):
        ...
        if params.reset_head:
            model.reset_metaoptnet_head(xs)
        ...
        opt = torch.optim.AdamW(model.parameters(), lr=params.ft_lr)
        ...
        for epoch in range(params.ft_epoch):
            ...
            for j in range(0, support_size, batch_size):
                ...
                loss = loss_fn(scores, y_batch) + params.alpha * ce_loss(attn, focus_batch)
                loss.backward()
                opt.step()
        del opt
